    ...
    <section class="bg-white pt-32 pb-8 antialiased">
        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
                ...
                @foreach ($purchases as $purchase)
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 items-center gap-x-4 gap-y-4 py-6 w-full">
                        ...
                        <div class="content-center">
                            <div class="flex items-center justify-end gap-2 sm:justify-start">
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    @endphp
                                    <span class="font-medium text-gray-900">Kode Unik</span>: {{ $kodeTampil }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="col-span-2 content-center sm:col-span-1 sm:justify-self-end">
                            <button class="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 sm:w-auto show-qr-btn" data-unique="{{ $purchase->unique_code }}">Check In</button>
                        </div>
                    </div>
                    @else
                    <div class="py-6">
          ...
          <div id="qrModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg p-6 max-w-xs w-full relative">
                <button id="closeQrModal" class="absolute top-2 right-2 text-gray-400 hover:text-gray-600 text-2xl">&times;</button>
                <h3 class="text-lg font-semibold mb-4 text-center">QR Code Tiket</h3>
                <div id="qrCodeContainer" class="flex justify-center mb-2"></div>
                <div id="qrInfo" class="text-center text-xs text-gray-500"></div>
            </div>
        </div>
        </div>
    </section>
...
document.querySelectorAll('.show-qr-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
        var kodeunik = btn.getAttribute('data-unique');
        var qr = new QRious({
            value: kodeunik,
            size: 200
        });
        ...
        document.getElementById('qrInfo').textContent = 'Kode Unik: ' + kodeunik;
...
window.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') document.getElementById('qrModal').classList.add('hidden');
});
</script>
</x-main-layout>